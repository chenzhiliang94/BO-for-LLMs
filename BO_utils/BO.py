...
def joint_opt_BO_LLM_generalized(
    ...
    # -------------------------
    input_X, input_X_between_0_1, fidelity = randomly_generate_data(what_to_optimize, data_domains, lora_max_num_layers, lora_rank_max, BO_params, seed=None) # keep seed fixed for initial point
    ...
    all_intermediate_results = []  # per-iteration intermediate eval_loss trajectory or step performances
    all_fidelity_levels = []
    ...
    for init_sample_idx in range(num_initial_random_samples):
        ...
        observed_perf, realized_perf, init_train_results = evaluate_single_configuration(
            input_X=input_X,
        ...
        # Store intermediate eval_loss trajectory or step performances
        intermediate_info = {}
        if "eval_loss" in init_train_results:
            intermediate_info["eval_loss_trajectory"] = init_train_results["eval_loss"]
        if "step_performances" in init_train_results:
            intermediate_info["step_performances"] = init_train_results["step_performances"]
        all_intermediate_results.append(intermediate_info)
    ...
    while itr < BO_params["BO_iterations"]:
        ...
        if "eval_loss" in train_results:
            intermediate_info["eval_loss_trajectory"] = train_results["eval_loss"]
        if eval_method == "performance" and 'performance_eval_callback' in dir():
            intermediate_info["step_performances"] = performance_eval_callback.step_performances
        all_intermediate_results.append(intermediate_info)
    ...
    if fidelity is not None:
        return GP_input, full_input, full_train_results_list, gp, all_fidelity_levels, full_train_results_list, all_intermediate_results
